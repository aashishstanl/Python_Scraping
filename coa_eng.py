...
try:
    ...
    with open("./usa_engineering.html") as fp:
        ...
        for index, item in enumerate(cost):
            ...
            item_.append('')     #enter the course type before running(e.i, Engineering or Educaiton etc)
...
# #creating dataframe
        ...
        #-----------------------------------uncomment below section for engineering COA and comment  all below ----------------------------------
        
        df.columns = ['University', 'b', 'Tuition', 'Duration', 'e', 'f', 'g', 'h', 'City', 'State', 'Type', 'Books_href']
        df = df[df['Tuition']!= "N/A"]
        df.drop(labels=['b','e','f','g', 'h'], axis=1, inplace=True)
        ...
        df.to_csv("./Engineering_final.csv", index=True)
